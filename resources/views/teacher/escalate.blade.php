...
@section('content')
    ...
    <div class="mb-6 rounded-xl border-2 border-slate-900 bg-slate-50 px-5 py-4">
        ...
        <p class="mt-1 text-lg text-slate-700">{{ $incident->category?->name }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('teacher.escalate.store') }}">
        ...
        <div class="space-y-3">
            <label class="flex min-h-[60px] cursor-pointer items-center gap-3 rounded-xl border-2 border-slate-300 px-5 py-3 text-lg hover:bg-slate-50 has-[:checked]:border-slate-900 has-[:checked]:bg-slate-50">
        ...
        <label class="mt-6 flex cursor-pointer items-start gap-3 rounded-xl bg-slate-100 px-4 py-3 text-base">
        ...
        <button type="submit"
                class="mt-5 flex min-h-[64px] w-full items-center justify-center rounded-xl bg-slate-900 px-5 py-4 text-lg font-semibold text-white transition active:scale-[.99] hover:bg-slate-800">
