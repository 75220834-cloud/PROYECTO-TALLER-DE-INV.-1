...
@php
    $bandStyles = [
        'high' => ['Alto', 'bg-rose-50 border-rose-200', 'text-rose-700'],
        'medium' => ['Medio', 'bg-amber-50 border-amber-200', 'text-amber-700'],
        'low' => ['Bajo', 'bg-slate-50 border-slate-200', 'text-slate-600'],
    ];
@endphp

<h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Por aula</h2>
...
<div class="mb-8 grid gap-3 md:grid-cols-2">
    @forelse ($rooms as $score)
        ...
        <div class="rounded-lg border {{ $bandBox }} p-4">
            ...
            <dl class="mt-3 space-y-1 border-t border-white/60 pt-3 text-sm">
                @foreach ($score->factors as $factor)
                    <div class="flex items-baseline justify-between gap-3">
                        <dt class="text-slate-700">
                            {{ $factor['label'] }}
                            <span class="text-slate-500">— {{ $factor['detail'] }}</span>
                        </dt>
                        <dd class="shrink-0 font-mono text-xs text-slate-600">
...
<h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Por aula y tipo de problema</h2>
...
<div class="grid gap-3 md:grid-cols-2">
    ...
    @empty
        <p class="text-sm text-slate-500">Sin pares con historial reciente.</p>
    @endforelse
</div>
@endsection
