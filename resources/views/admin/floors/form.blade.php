...
<form method="POST" action="{{ $floor->exists ? route('admin.floors.update', $floor) : route('admin.floors.store') }}"
    ...
    <x-field label="Pabellón" name="building_id" :required="true">
        <select id="building_id" name="building_id" required class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm">
    ...
    <label class="flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $floor->exists ? $floor->is_active : true)) class="rounded border-slate-300">
        Activo
    </label>
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('admin.floors.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Cancelar</a>
    </div>
</form>
@endsection
