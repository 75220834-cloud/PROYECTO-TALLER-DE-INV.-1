...
<div class="mb-5 rounded-lg border {{ $coverage['percent'] === 100 ? 'border-ok bg-ok-container' : 'border-warn bg-warn-container' }} px-5 py-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <p class="text-sm font-medium {{ $coverage['percent'] === 100 ? 'text-on-ok-container' : 'text-on-warn-container' }}">
                Cobertura visual: {{ $coverage['covered'] }} de {{ $coverage['total'] }} pasos
            </p>
            <p class="mt-1 text-sm {{ $coverage['percent'] === 100 ? 'text-on-ok-container' : 'text-on-warn-container' }}">
        ...
        <div class="flex items-center gap-3">
            <p class="text-3xl font-bold {{ $coverage['percent'] === 100 ? 'text-ok' : 'text-warn' }}">
                {{ $coverage['percent'] }}%
            </p>
            @if ($coverage['percent'] < 100)
                <a href="{{ route('admin.media.coverage') }}"
                   class="rounded-md border border-warn bg-surface-lowest px-3 py-2 text-sm font-medium text-on-warn-container">
                    Ver qué falta
                </a>
            @endif
        </div>
...
<div class="mb-5 flex flex-wrap items-center justify-between gap-3">
    <form method="GET" class="flex flex-wrap items-center gap-2">
        ...
        <select name="scope" data-auto-submit class="rounded-md border-outline-variant px-3 py-2 text-sm">
...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    @forelse ($assets as $asset)
        <div class="overflow-hidden rounded-lg border border-outline-variant bg-surface-lowest {{ $asset->is_active ? '' : 'opacity-60' }}">
            ...
            <div class="p-3">
                <p class="truncate font-mono text-xs text-on-surface-variant">{{ $asset->code }}</p>
                ...
                <div class="mt-3 flex items-center gap-3 text-sm">
                    <a href="{{ route('admin.media.edit', $asset) }}" class="text-on-surface-variant underline underline-offset-2">Editar</a>
                    ...
                    @if (! $asset->is_active)
                        <span class="ml-auto rounded bg-outline-variant px-1.5 py-0.5 text-xs text-on-surface-variant">oculta</span>
                    @endif
                </div>
            </div>
        </div>
    @empty
        <p class="text-sm text-on-surface-variant sm:col-span-2 lg:col-span-3 xl:col-span-4">
