...
<form method="POST" action="{{ $room->exists ? route('admin.rooms.update', $room) : route('admin.rooms.store') }}"
    ...
    <x-field label="Piso" name="floor_id" :required="true">
        <select id="floor_id" name="floor_id" required class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <x-field label="Criticidad" name="criticality" :required="true" help="Influye en la prioridad del ticket.">
            <select id="criticality" name="criticality" required class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 text-sm">
    ...
    <label class="flex items-center gap-2 text-sm text-slate-700">
        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $room->exists ? $room->is_active : true)) class="rounded border-slate-300">
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('admin.rooms.index') }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Cancelar</a>
    </div>
</form>
@endsection
