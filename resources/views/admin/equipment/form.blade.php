...
<form method="POST" action="{{ $item->exists ? route('admin.equipment.update', $item) : route('admin.equipment.store') }}"
    ...
    <x-field label="Aula" name="room_id" :required="true">
        <select id="room_id" name="room_id" required class="mt-1 block w-full rounded-md border-outline-variant px-3 py-2 text-sm">
    ...
    <x-field label="Tipo de equipo" name="equipment_type_id" :required="true">
        <select id="equipment_type_id" name="equipment_type_id" required class="mt-1 block w-full rounded-md border-outline-variant px-3 py-2 text-sm">
            <option value="">Selecciona…</option>
            @foreach ($types as $t)
    ...
    <div class="grid grid-cols-2 gap-4">
        <x-field label="Estado" name="status" :required="true">
            <select id="status" name="status" required class="mt-1 block w-full rounded-md border-outline-variant px-3 py-2 text-sm">
    ...
    <label class="flex items-center gap-2 text-sm text-on-surface-variant">
        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $item->exists ? $item->is_active : true)) class="rounded border-outline-variant">
        Activo
    </label>

    <div class="flex gap-3 pt-2">
        <button type="submit" class="btn-primary focus-ring px-4 py-2 text-sm font-medium hover:bg-on-surface">Guardar</button>
        <a href="{{ route('admin.equipment.index') }}" class="rounded-md border border-outline-variant px-4 py-2 text-sm text-on-surface-variant hover:bg-surface-low">Cancelar</a>
    </div>
</form>
@endsection
